...
\begin{enumerate}
\item \code{(FunctionDef params body)}中，\code{body}处于尾位置。
\item 如果 \code{(let x e body)} 处于尾位置，则\code{body}处于尾位置。
\end{enumerate}

我们使用两个相互递归的函数\code{explicate-tail} 和 \code{explicate-assign}来实现：
\begin{lstlisting}
...
\code{explicate-tail}函数应用于位于尾位置的表达式，
而\code{explicate-assign} 函数应用于出现在 \key{let} 右侧的表达式。
二者都返回一串四元式。
